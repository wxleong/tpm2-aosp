...
typedef keymaster::UniquePtr<EVP_PKEY, EVP_PKEY_Delete> Unique_EVP_PKEY;
...
typedef keymaster::UniquePtr<RSA, RSA_Delete> Unique_RSA;
...
typedef keymaster::UniquePtr<PKCS8_PRIV_KEY_INFO, PKCS8_PRIV_KEY_INFO_Delete> Unique_PKCS8_PRIV_KEY_INFO;

typedef keymaster::UniquePtr<keymaster0_device_t> Unique_keymaster_device_t;

typedef keymaster::UniquePtr<CK_BYTE[]> Unique_CK_BYTE;

typedef keymaster::UniquePtr<CK_ATTRIBUTE[]> Unique_CK_ATTRIBUTE;
...
typedef keymaster::UniquePtr<ByteArray> Unique_ByteArray;
...
public:
    CryptoSession(CK_SESSION_HANDLE masterHandle) :
        ...
        if (openSessionRV != CKR_OK || subsessionHandle == CK_INVALID_HANDLE) {
            ...
            ALOGI("Opening subsession 0x%x", subsessionHandle);
    ...
    ~CryptoSession() {
        /*if (mSubsession != CK_INVALID_HANDLE) {
            ...
            ALOGI("Closing subsession 0x%x: 0x%x", mSubsession, rv);
...
public:
    ...
    ~ObjectHandle() {
        /*if (mHandle != CK_INVALID_HANDLE) {
            ...
            if (rv != CKR_OK) {
                ALOGE("Couldn't close object handle 0x%x: 0x%x", mHandle, rv);
            } else {
                ALOGI("Closing object handle 0x%x", mHandle);
...
static int find_single_object(const uint8_t* obj_id, const size_t obj_id_length,
    ...
    ALOGI("Found %d object 0x%x : class 0x%x", tmpCount, tmpHandle, obj_class);
...
static int keyblob_restore(const CryptoSession* session, const uint8_t* keyBlob,
    ...
    ALOGE("Good key version %d", keyVersion);
...
static void tee_close3(const keymaster0_device_t* device) {
#ifndef PKCS11_SINGLETON
    keymaster0_device_t* dev = const_cast<keymaster0_device_t*>(device);

    if (dev != NULL) {
        CK_SESSION_HANDLE handle = reinterpret_cast<CK_SESSION_HANDLE>(dev->context);
    ...
    dev->context = reinterpret_cast<void*>(CK_INVALID_HANDLE);
#endif
}

static int tee_close2(const keymaster0_device_t* dev, int rv) {
...
#ifdef LATE_OPEN
/**
 * To delay PKCS11 token initialization.
 * android.hardware.keymaster@3.0-impl (hardware/interfaces/keymaster/3.0/default/KeymasterDevice.cpp) is loaded at early boot
 * TPM simulator tcp port only available at later boot
 */
static int tee_late_open(const keymaster0_device_t* device) {
    CK_RV rv;
    ...
    if (reinterpret_cast<CK_SESSION_HANDLE>(dev->context) != CK_INVALID_HANDLE)
        return 0;

    rv = C_Initialize(NULL);
    if (rv != CKR_OK) {
        ALOGE("Error initializing TEE: 0x%x", rv);
        return tee_close2(dev, -1);
    }
    ...
    if (rv != CKR_OK) {
        (void) C_Finalize(NULL_PTR);
        ALOGE("Error getting information about TEE during initialization: 0x%x", rv);
        return tee_close2(dev, -1);
    }
    ...
    rv = C_OpenSession(CKV_TOKEN_USER,
            CKF_SERIAL_SESSION | CKF_RW_SESSION,
            NULL,
            NULL,
            &sessionHandle);

    if (rv != CKR_OK || sessionHandle == CK_INVALID_HANDLE) {
        (void) C_Finalize(NULL_PTR);
        ALOGE("Error opening primary session with TEE: 0x%x", rv);
        return tee_close2(dev, -1);
    }

    CK_UTF8CHAR pin[] = CKV_TOKEN_USERPIN;

    rv = C_Login(sessionHandle, CKU_USER, pin, sizeof(pin) - 1);

    if (rv != CKR_OK) {
        (void) C_Finalize(NULL_PTR);
        ALOGE("Error on login: 0x%x", rv);
        return tee_close2(dev, -1);
    }
...
static int tee_generate_keypair(const keymaster0_device_t* dev,
    ...
    ALOGI("tee_generate_keypair start");

#ifdef LATE_OPEN
    if (tee_late_open(dev)) {
        ALOGE("TEE late init failed");
        return tee_close2(dev, -1);
    }
#endif

    if (type != TYPE_RSA) {
        ALOGE("Unknown key type %d", type);
        return tee_close2(dev, -1);
    }

    if (key_params == NULL) {
        ALOGE("generate_keypair params were NULL");
        return tee_close2(dev, -1);
    }
    ...
    for (size_t i = 0; i < sizeof(publicExponent); i++) {
        ...
        ALOGE("publicExponent %x %d %d %d", publicExponent[offset + 1], sizeof(uint64_t), sizeof(publicExponent), CHAR_BIT);
    }
    ALOGE("publicExponent %x %x %x %x %x %x %x %x", publicExponent[0], publicExponent[1], publicExponent[2], publicExponent[3], publicExponent[4], publicExponent[5], publicExponent[6], publicExponent[7]);
    ...
    CK_ATTRIBUTE publicKeyTemplate[] = {
            ...
            {CKA_PUBLIC_EXPONENT, publicExponent + 4, sizeof(publicExponent) - 4},
    ...
    ALOGI("public handle = 0x%x, private handle = 0x%x", publicKey.get(), privateKey.get());

    tee_close3(dev);

    ALOGI("tee_generate_keypair end");
...
static int tee_import_keypair(const keymaster0_device_t* dev,
    ...
    if (EVP_PKEY_base_id(pkey.get()) != EVP_PKEY_RSA) {
        ALOGE("Unsupported key type: %d", EVP_PKEY_base_id(pkey.get()));
        return tee_close2(dev, -1);
    }
    ...
    Unique_ByteArray modulus(bignum_to_array(n));
    if (modulus.get() == NULL) {
        ALOGE("Could not convert modulus to array");
        return tee_close2(dev, -1);
    }

    Unique_ByteArray publicExponent(bignum_to_array(e));
    if (publicExponent.get() == NULL) {
        ALOGE("Could not convert publicExponent to array");
        return tee_close2(dev, -1);
    }
    ...
    Unique_ByteArray privateExponent(bignum_to_array(d));
    if (privateExponent.get() == NULL) {
        ALOGE("Could not convert private exponent");
        return tee_close2(dev, -1);
    }
...
#define PRIV_ATTRIB_EXTENDED_NUM (PRIV_ATTRIB_NORMAL_NUM + 5)
    ...
    bool has_extra_data = (p != NULL) && (q != NULL) && (dmp1 != NULL) &&
            (dmq1 != NULL) && (iqmp != NULL);
    ...
    if (has_extra_data) {
        prime1.reset(bignum_to_array(p));
        if (prime1->get() == NULL) {
            ALOGE("Could not convert prime1");
            return tee_close2(dev, -1);
        }
        set_attribute(&privateKeyTemplate[templateOffset++], CKA_PRIME_1, prime1->get(),
                prime1->length());

        prime2.reset(bignum_to_array(q));
        if (prime2->get() == NULL) {
            ALOGE("Could not convert prime2");
            return tee_close2(dev, -1);
        }
        set_attribute(&privateKeyTemplate[templateOffset++], CKA_PRIME_2, prime2->get(),
                prime2->length());

        exp1.reset(bignum_to_array(dmp1));
        if (exp1->get() == NULL) {
            ALOGE("Could not convert exponent 1");
            return tee_close2(dev, -1);
        }
        set_attribute(&privateKeyTemplate[templateOffset++], CKA_EXPONENT_1, exp1->get(),
                exp1->length());

        exp2.reset(bignum_to_array(dmq1));
        if (exp2->get() == NULL) {
            ALOGE("Could not convert exponent 2");
            return tee_close2(dev, -1);
        }
        set_attribute(&privateKeyTemplate[templateOffset++], CKA_EXPONENT_2, exp2->get(),
                exp2->length());

        coeff.reset(bignum_to_array(iqmp));
        if (coeff->get() == NULL) {
            ALOGE("Could not convert coefficient");
            return tee_close2(dev, -1);
        }
...
static int tee_get_keypair_public(const keymaster0_device* dev,
    ...
    ALOGI("tee_get_keypair_public start");
    ...
    if (x509_data == NULL || x509_data_length == NULL) {
        ALOGE("Provided destination variables were null");
        return tee_close2(dev, -1);
    }
    ...
    CK_RV rv = C_GetAttributeValue(session.get(), publicKey.get(), attributes,
            sizeof(attributes)/sizeof(CK_ATTRIBUTE));
    if (rv != CKR_OK) {
        ALOGE("Could not query attribute value sizes: 0x%02x", rv);
        return tee_close2(dev, -1);
    }
    ...
    rv = C_GetAttributeValue(session.get(), publicKey.get(), attributes,
            sizeof(attributes) / sizeof(CK_ATTRIBUTE));
    if (rv != CKR_OK) {
        ALOGE("Could not query attribute values: 0x%02x", rv);
        return tee_close2(dev, -1);
    }

    ALOGI("modulus is %d (ret=%d), exponent is %d (ret=%d)",
    ...
    n = BN_bin2bn(reinterpret_cast<const unsigned char*>(modulus.get()), modulus.length(),
            NULL);
    if (n == NULL) {
    ...
    e = BN_bin2bn(reinterpret_cast<const unsigned char*>(exponent.get()), exponent.length(),
            NULL);
    if (e == NULL) {
        logOpenSSLError("tee_get_keypair_public");
        return tee_close2(dev, -1);
    ...
    keymaster::UniquePtr<uint8_t> key(static_cast<uint8_t*>(malloc(len)));
    if (key.get() == NULL) {
    ...
    ALOGI("Length of x509 data is %d", len);
    *x509_data_length = len;
    *x509_data = key.release();

    ALOGI("tee_get_keypair_public end");

    return tee_close2(dev, 0);
}

static int tee_delete_keypair(const keymaster0_device_t* dev,
            const uint8_t* key_blob, const size_t key_blob_length) {
    ALOGI("tee_delete_keypair start key_blob_length %d", key_blob_length);
    ...
    if (rv != CKR_OK) {
        ALOGE("Could destroy private key object: 0x%02x", rv);
        return tee_close2(dev, -1);
    }
    ...
    if (rv != CKR_OK) {
        ALOGE("Could destroy public key object: 0x%02x", rv);
        return tee_close2(dev, -1);
    }

    ALOGI("tee_delete_keypair end");
...
static int tee_sign_data(const keymaster0_device_t* dev,
    ...
    ALOGI("tee_sign_data(%p, %p, %llu, %p, %llu, %p, %p)", dev, key_blob,
    ...
    if (params == NULL) {
        ALOGE("Signing params were null");
        return tee_close2(dev, -1);
    }
    ...
    if (rv != CKR_OK) {
        ALOGE("C_SignInit failed: 0x%x", rv);
        return tee_close2(dev, -1);
    }
    ...
    rv = C_Sign(session.get(), (unsigned char *)data, dataLength, signature, &signatureLength);
    if (rv != CKR_OK) {
        ALOGE("C_SignFinal failed: 0x%x", rv);
        return tee_close2(dev, -1);
    }

    keymaster::UniquePtr<uint8_t[]> finalSignature(new uint8_t[signatureLength]);
    ...
    *signedDataLength = static_cast<size_t>(signatureLength);
    ...
    ALOGI("tee_sign_data(%p, %p, %llu, %p, %llu, %p, %p) => %p size %llu", dev, key_blob,
            (unsigned long long) key_blob_length, data, (unsigned long long) dataLength, signedData,
...
static int tee_verify_data(const keymaster0_device_t* dev,
    ...
    ALOGI("tee_verify_data(%p, %p, %llu, %p, %llu, %p, %llu)", dev, keyBlob,
    ...
    if (params == NULL) {
        ALOGE("Verification params were null");
        return tee_close2(dev, -1);
    }
    ...
    if (rv != CKR_OK) {
        ALOGI("C_VerifyInit failed: 0x%x", rv);
        return tee_close2(dev, -1);
    }

    // This is a bad prototype for this function. C_Verify should have only const args.
    rv = C_Verify(session.get(), (unsigned char *)signedData, signedDataLength,
            const_cast<unsigned char*>(signature), signatureLength);
    if (rv != CKR_OK) {
        ALOGI("C_Verify failed: 0x%x", rv);
        return tee_close2(dev, -1);
    }

    ALOGI("tee_verify_data end");
...
static int tee_open(const hw_module_t* module, const char* name,
    ...
    dev->flags = KEYMASTER_SUPPORTS_EC; // to indicate addional ALGO (e.g., KEYMASTER_SUPPORTS_EC, KEYMASTER_SUPPORTS_DSA), RSA is supported by default
...
#ifndef LATE_OPEN
    if (tee_late_open((const keymaster0_device_t*)dev.get()))
        return -1;
#else
    CK_SESSION_HANDLE sessionHandle = CK_INVALID_HANDLE;
